...
void MultilayerPerceptron::forwardPropagate() {
	...
	if(outputFunction == 1){
		double sum = 0;
		for (size_t i = 0; i < layers[nOfLayers].nOfNeurons; i++)
			sum += exp( layers[nOfLayers].neurons[i].out );
		for (size_t i = 0; i < layers[nOfLayers].nOfNeurons; i++)
			layers[nOfLayers].neurons[i].out = exp (layers[nOfLayers].neurons[i].out) / sum;
	}
}
